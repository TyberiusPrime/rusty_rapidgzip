...
impl<'a> DeflateDecoder<'a>
{
    ...
    /// LOCAL PATCH (rusty-rapidgzip): byte offset within the input at which
    /// decoding ended — the byte-aligned start of any trailer following the
    /// DEFLATE stream. Mirrors the internal footer-locating expression used by
    /// `decode_zlib`/`decode_gzip`. Valid only after a successful
    /// `decode_deflate`. Lets the caller find a gzip member's trailer (and thus
    /// the next member) without zune parsing the gzip wrapper itself.
